...
class SendMailHandler(webapp.RequestHandler):
    def post(self):
        ...
        mail.send_mail(
                        sender = "lhh1990@gmail.com",
                        to = "lihaohua90 <lihaohua90@gmail.com>",
...
class GetByKeyHandler(webapp.RequestHandler):
    def get(self):
        ...
        if desktop:
            ...
            self.response.out.write('Error: key not found.')
class CleanHandler(webapp.RequestHandler):
    def get(self):
        ...
        try:
            ...
            count = 500
        desktops = db.GqlQuery('SELECT __key__ FROM Desktop ORDER BY time')  # 时间从小到大排列, 从老到新
        item_count = desktops.count()
        self.response.out.write('Item Count: %d<br>' %item_count)
        if item_count > count:
            clean_count = item_count - count
            print 'Cleaning %d items..<br>' %clean_count
            desktops = db.GqlQuery('SELECT __key__ FROM Desktop ORDER BY time LIMIT %d' %clean_count)  # 时间从小到大排列, 从老到新
            db.delete(desktops)
            self.response.out.write('Clean OK.')
        else:
            self.response.out.write('not too many. :)')
def main():
    application = webapp.WSGIApplication(
        [
        ('/upload', UploadHandler),
        ('/get', GetHandler),
        ('/getbytime', GetByTimeHandler),
        ('/getzip', GetZipHandler),
        ('/getbykey', GetByKeyHandler),
        ('/getinterval', GetIntervalHandler),
